...
For more advanced customization, you can modify properties directly in the code. You can also collect booking data from an embedded Booking page using the [client-side API](/scheduleonce/developers/client-side-api/embedded-booking-calendar-events/) for use cases such as creating a custom confirmation page, enabling client-side integrations, or enriching Customer data profiles.
