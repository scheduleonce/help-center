...
The [Link header](https://developer.mozilla.org/en-US/scheduleonce/developers/Web/HTTP/Headers/Link) includes a `rel` parameter, where relation-types describes the relation of the linked page to the current page of results. The value can either be `previous` or `next`.
