...
description: Generate and use API keys to authenticate requests to the ScheduleOnce Booking Pages API with secure bearer token authentication.
products:
  - scheduleonce
---

To interact with the **ScheduleOnce REST API**, you must authenticate every request using a unique **API Key**. For your security, all communication must occur over **HTTPS**; requests made over plain HTTP will be rejected.

ScheduleOnce utilizes industry-standard cryptographic practices to protect your credentials.

- **Hashed Key Storage:** ScheduleOnce does not store API keys in plaintext. Because we only store a secure cryptographic hash, your key is **displayed only once** upon generation. It cannot be retrieved again by any user or by connecting with support team.
...
1. Log in to your ScheduleOnce account and click the **gear icon** located in the top-right corner of the page.
...
Include your API key in the API-Key header of every HTTP request. If the key is missing or invalid, the API will return an error response to assist with troubleshooting. [**Learn more about error responses**](/scheduleonce/developers/overview/error-handling/).
...
Once you have your key, test it by making a request to our [**validation endpoint**](/scheduleonce/developers/api/#tag/authentication/GET/test) to confirm your connection is active. If the key is missing or invalid, the API will return a 401 Unauthorized error.
...
- **Lost Keys:** If you lose an API key, the support team cannot recover it for you. You must delete the lost key, generate a new one, and update your integration.
